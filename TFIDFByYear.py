...
# Compile only the year and lyrics (Note: These inputs can be modified if different metadata is sought (e.g. 
# artists/genres instead of time periods))
...
with open(filename,'rt') as lyricFile:
    ...
    for row in lyricReader:
        try:
            lyric = row[3].lower() # Mind the order when using _LyricWikia(2) files
            year = int(row[1]) # Mind the order when using _LyricWikia(2) files
            rawLyrics.append((year,lyric))
...
# For every year, consolidate *all* lyrics (from all songs) into a single string, then combine that string into a tuple
# with the year

lyricsByPeriod = []
for year in allYears:
    if int(year) < 1950: # Don't consider anything before 1950 (LyricWikia has records from the 1700s and 1800s!)
        pass
    else:
        allLyrics = ""
        for song in rawLyrics:
            if int(song[0]) == year:
                ...
                pass
        lyricsByPeriod.append((year,allLyrics))
...
# Iterate the Dictionary Builder function across all time periods and create a new list of tuples pairing each year with
...
def TwentyLargestTFIDFByComponent(tfd):
    outfilename = "%s" % letter + "_YearTFIDF_LyricWikia.csv" # commented out when assessing the entire data set
    #outfilename = "AllRecords_YearTFIDF_LyricWikia.csv"
    s=0
...
# Completion time for letter X (1654 KB/1675 records) with terms with less than 2 occurrences discarded: 3.03 seconds

# Completion time for letter P (37,000 KB/35,258 records) with terms with less than 2 occurrences discarded: 844.29 seconds
# (approximately 14 minutes 5 seconds; longer than other similar tests)

# Completion time for entire LyricWikia Data Set (893,401 KB) with terms with less than 10 occurrences discarded: TBD 
# (approximately TBD)